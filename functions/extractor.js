...
// Cloud / Serverless detection (Firebase Cloud Functions or Vercel Linux)
...
// ffmpeg detection: Use ffmpeg-static if available, then system winget path
let ffmpegDir = '';
let hasFfmpeg = false;
try {
  ...
  if (ffmpegStatic && fs.existsSync(ffmpegStatic)) {
    ...
    ffmpegDir = path.dirname(ffmpegStatic);
    hasFfmpeg = true;
  }
} catch (_) {}

if (!hasFfmpeg && !isCloud) {
  const localFfmpeg = 'C:\\Users\\G Shreeniketh\\AppData\\Local\\Microsoft\\WinGet\\Packages\\Gyan.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe\\ffmpeg-9.0.1-full_build\\bin';
  if (fs.existsSync(path.join(localFfmpeg, 'ffmpeg.exe'))) {
    ffmpegDir = localFfmpeg;
    hasFfmpeg = true;
  }
}

// yt-dlp path detection: prefer bundled bin/yt-dlp on cloud, local Windows install otherwise
let ytDlpPath = 'yt-dlp';
const bundledYtDlp = path.join(__dirname, 'bin', 'yt-dlp');
...
const galleryDlPath = (!isCloud && fs.existsSync(defaultGalleryDl)) ? defaultGalleryDl : 'gallery-dl';
const pythonBin = isCloud ? 'python3' : 'C:\\Users\\G Shreeniketh\\AppData\\Local\\Programs\\Python\\Python313\\python.exe';
...
const nodeBin = isCloud ? 'node' : 'C:\\Program Files\\nodejs\\node.exe';
const userCookiePath = isCloud ? path.join('/tmp', 'user_cookies.txt') : path.join(__dirname, 'user_cookies.txt');
      ...
      if (isCarousel) {
        ...
        if (items.length > 0) {
          const playlistResult = {
            ...
            uploader: unescapeHtml(ytDlpData.uploader || ytDlpData.channel || 'Media Collection'),
            thumbnail: items[0]?.thumbnail || '',
            itemCount: items.length,
            webpage_url: targetUrl,
